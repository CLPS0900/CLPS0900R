...
#'
#' @export
run.onesample.ztest <- function(mu=100,sigma=15,n=10,observed.mean=110){
  ...
  sem <- sigma/sqrt(n)
  mlo <- round((observed.mean - 2*sem),0)
  mhi <- round((observed.mean + 2*sem),0)
